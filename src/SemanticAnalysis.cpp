...
Attribute SemanticAnalysis::checkAttribute(Token& attr){
	...
	// find the relation that consists attr as attribute name
	...
	throw SemanticError("Attribute "+ attr.value +" does not exist in any relation");
}

Attribute SemanticAnalysis::checkAttribute(SqlAttribute& attr){
	
	Attribute resolvedAttr;
	SqlBinding temp; // table which consists attr
	if(relations.find(attr.binding.value) != relations.end()){
		...
		throw SemanticError("Attribute "+ attr.attribute.value +" does not exist in any relation");
	}
